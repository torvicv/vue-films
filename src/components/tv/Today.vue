<script>
  ...
  export default {
    ...
    async mounted(props) {
      const url = 'https://api.themoviedb.org/3/tv/airing_today?language=en-US&page='+this.$props.page
      await axios
       .get(url, headers)
       .then((response) => {
    ...
    watch: {
        async page(val) {
            const url = 'https://api.themoviedb.org/3/tv/airing_today?language=en-US&page='+val
            await axios
            .get(url, headers)
            .then((response) => {
                this.films = response.data.results;
...
<template>
  <div class="flex items-center w-full flex-col bg-slate-50 mb-10">
    <h1 class="text-6xl text-center font-extrabold my-8">Estrenos hoy</h1>
