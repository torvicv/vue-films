<script>
  ...
  export default {
    ...
    async mounted(props) {
      const url = 'https://api.themoviedb.org/3/tv/top_rated?language=en-US&page='+this.$props.page
      await axios
       .get(url, headers)
       .then((response) => {
    ...
    watch: {
        async page(val) {
            const url = 'https://api.themoviedb.org/3/tv/top_rated?language=en-US&page='+val
            await axios
            .get(url, headers)
            .then((response) => {
                this.films = response.data.results;
